        ...
        <div class="cart-foot-wrap">
          <div class="cart-foot-l">
            <div class="item-all-check">
              <a href="javascript:void 0">
                    <span class="item-check-btn" v-bind:class="{'check':checkAllFlag}" @click="checkAll(!checkAllFlag)">
                      <svg class="icon icon-ok"><use xlink:href="#icon-ok"></use></svg>
                    </span>
              </a>
            </div>
          ...
          <div class="cart-foot-r">
            ...
            <div class="next-btn-wrap">
              <router-link :to="`/addr`"><a class="btn btn--red" style="width: 200px">结账</a></router-link>
            </div>
          </div>
        </div>
      </div>
    ...
    methods:{
        ...
        selectedProduct : function(item){
            if (typeof item.checked == 'undefined') {
                //Vue.set(item,"checked",true);//全局注册一个属性checked
                this.$set(item,"checked",true);  //局部注册
            }else{
                item.checked = !item.checked;   //取消选中
            }
            this.calcTotalPrice();
        },
